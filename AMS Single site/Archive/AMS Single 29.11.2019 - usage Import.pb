...
;/ 
;/ Added new reporting systrem, customizable layouts with fields selection.
;/ General History types entries now controlled through customizable table.
;/ Rudimentary usage System
...
If Multi_Site_Mode = 0
  Program_Version.s = " V1.63 - Single-Site"  ;/DNT
Else
  Program_Version.s = " V1.63 - Multi-Site" ;/DNT
EndIf
...
Procedure Database_CheckVersion(Quiet = #True)
  ...
  If Version < 14 ;/ skip a couple of versions to puh through updates on 'inbetween' releases...
    Debug "*** HR  *** New database version is 13...."
    Database_Update(#Databases_Master,"DROP TABLE [AMS_ReportLayouts];",#PB_Compiler_Line) ;/DNT
    ...
    Database_Update(#Databases_Master,"Create TABLE [AMS_General_History_Types] ([ID] INTEGER PRIMARY KEY, [RollType] Int, [Name] Char);",#PB_Compiler_Line)                                          ;/DNT
    ...
    Database_Update(#Databases_Master,"ALTER TABLE AMS_Roll_Data ADD [Usage] Char",#PB_Compiler_Line)                                                  ;/DNT
    Database_Update(#Databases_Master,"ALTER TABLE AMS_Roll_Master ADD [Usage] Char",#PB_Compiler_Line)                                                  ;/DNT
    Database_Update(#Databases_Master,"ALTER TABLE AMS_Roll_Master ADD [Current_Usage] Char",#PB_Compiler_Line)                                                  ;/DNT
    
    ;/ obtain a list of pre-existing types and add them to the AMS_General_History_Types table
    DatabaseQuery(#Databases_Master,"SELECT DISTINCT Type from AMS_General_History Order by Type;")
    While NextDatabaseRow(#Databases_Master)  
      TypeCount + 1
    ...
    ;/ if there are no preset types, add a couple of defaults...
    If TypeCount = 0
      Database_Update(#Databases_Master, "INSERT INTO AMS_General_History_Types (Name) VALUES ('Cleaned');",#PB_Compiler_Line)                                  ;/DNT
      Database_Update(#Databases_Master, "INSERT INTO AMS_General_History_Types (Name) VALUES ('Damaged');",#PB_Compiler_Line)                                  ;/DNT
    EndIf
    
    AddedText + "Version 14 New Layout options added."+Chr(10)                                                                                                                                       ;/DNT
    ...
    Txt + "BEGIN" + #CRLF$
    Txt + "UPDATE AMS_Roll_Master SET Current_Usage = (SELECT (SELECT Sum(Usage) from AMS_Roll_Data where RollID = old.RollID) + (SELECT Sum(Usage) from AMS_Roll_Master where ID = 1)) Where ID = old.RollID;" + #CRLF$
    Txt + "END;"    
    Database_Update(#Databases_Master,txt,#PB_Compiler_Line)                                                  ;/DNT
    ...
    AddedText + "Version 14: Usage fields added to support graphing by usage."+Chr(10)   ;/DNT
    NewVersion = 14
...
Procedure Redraw_RollID(RollID.i, Force.i=0)
  ...
  SQL.s + "Operator, ReadingDate, Vol1,Vol2,Vol3,Vol4,Vol5,Volume, Capacity, Variance, Depth, Current_Depth, Usage"
...
Procedure Redraw_Graph(RollID.i)
    ...
    Select GraphType
      Case #GraphType_Volume, #GraphType_Usage
        ...
        Select GraphType
          ...
          Case #GraphType_Usage
            Usage(MyLoop) = GetDatabaseLong(#Databases_Master,5)
            DateString + NumberToRationlizedString(Usage(MyLoop)) + Seperator
        EndSelect
      ...
      Case #GraphType_Volume, #GraphType_Usage
        ...
        Select GraphType
          Case #GraphType_Volume
            DateString + UnitConversion_Date(GetDatabaseLong(#Databases_Master,5), 1) + Seperator
          Case #GraphType_Usage
            Usage(MyLoop) = Usage(MyLoop - 1) + GetDatabaseLong(#Databases_Master,5) 
            DateString + NumberToRationlizedString(Usage(MyLoop)) + Seperator
        EndSelect
  ...
  If RMC_CreateChart(GadgetID(#Gad_Graph_Image), ChartID, 0, 0, GadgetWidth(#Gad_Graph_Image)-3, GadgetHeight(#Gad_Graph_Image)-3,0,0,1) = #RMC_NO_ERROR
    ...
    StartDrawing(ImageOutput(#Image_Graph))
      ...
      For y = 53 To 559
        If Point(74,Y) = 33023 : DrawText(ImageWidth(#Image_Graph)-80,Y-17,Str(System\Settings_Capacity_Good)+"%",0) : EndIf
        If Point(74,Y) = 255 : DrawText(ImageWidth(#Image_Graph)-80,Y-17,Str(System\Settings_Capacity_Bad)+"%",0) : EndIf
      Next
...
Procedure Report_Generate(SiteID.i,GroupID.i)
  ...
  Protected HasCommentText.a, HasGHText.a, AdTxt.s, IsGood.a
  ForEach ReportData()
    Txt.s = ""
...
Procedure.f String_UsageComment_Breakdown() ;/ breakdown individual lines from comments box - check for usage entry, set value and remove if present.
 ...
 ; Txt + "Comment lines: " + Str(Lines) + #CRLF$
  ...
  For MyLoop = 0 To Lines
    ...
    ;If FindString(UCase(Comment(MyLoop)),"USAGE") Or FindString(UCase(Comment(MyLoop)),"DISTANCE") Or FindString(UCase(Comment(MyLoop)),"DISTANZ")
    If UCase(Left(Trim(Comment(MyLoop)),2)) = "PD"  Or UCase(Left(Trim(Comment(MyLoop)),2)) = "DD" ; Or FindString(UCase(Comment(MyLoop)),"DISTANZ")
      TmpComment = UCase(Comment(MyLoop))
      TmpComment = ReplaceString(TmpComment,"PD","")
      TmpComment = ReplaceString(TmpComment,"DD","")
      ;TmpComment = ReplaceString(TmpComment,"DISTANCE","")
      ;TmpComment = ReplaceString(TmpComment,"DISTANZ","")
      ;TmpComment = ReplaceString(TmpComment,":","")
      TmpComment = Trim(TmpComment)
      Usage = ValF(TmpComment)
      
      ;/ no usage, so possibly a false negative...
      If Usage = 0 : CommentS + Comment(MyLoop)+ #CRLF$ : EndIf
    Else
      CommentS + Comment(MyLoop)+ #CRLF$
    EndIf
  Next
  
  AMS_Import\Comment = CommentS
  ;Txt + "Comment Out: " + AMS_Import\Comment + #CRLF$
 ; Txt + "Usage: " + StrF(Usage) + #CRLF$
  
  ;MessageRequester("Import",Txt)
  
  ProcedureReturn Usage
...
Procedure.i Import_LoadAMS()
  ...
  With AMS_Import ;/ split the string into its component parts (tab delimited)
    ...
    If \Header = "AMS2" Or \Header = "AMS3"
      ...
      \Usage = String_UsageComment_Breakdown()
...
Procedure.i Import_AMS(NoLoad.i = 0) ;/ NoLoad for Master Import, to prevent reloading of asset
  ...
  If Date = 0 ;/ Assume no Original import, so Import to Roll Master
    ...
    If AMS_Import\Usage > 0 : Txt.S + ", Usage = "+StrF(AMS_Import\Usage) : EndIf
  ...
  Else  ;/ Assume original import already on, so import to history list
    ...
    Select AMS_Import\SamplesUsed
      Case "1"
        Txt.S + "ReadingDate, Operator, Vol3, AniCAM_Config, Depth, Usage, HistTopSnapImage) VALUES ("
        Txt.S + Str(System\Selected_Roll_ID)+","+Str(AMS_Import\DateNum)+",'"+AMS_Import\Operator+"',"+AMS_Import\Sample1
        
      Case "2"
        Txt.S + "ReadingDate, Operator, Vol2, Vol4, AniCAM_Config, Depth, Usage, HistTopSnapImage) VALUES ("
        Txt.S + Str(System\Selected_Roll_ID)+","+Str(AMS_Import\DateNum)+",'"+AMS_Import\Operator+"',"+AMS_Import\Sample1+","+AMS_Import\Sample2
        
      Case "3"
        Txt.S + "ReadingDate, Operator, Vol1, Vol3, Vol5, AniCAM_Config, Depth, Usage, HistTopSnapImage) VALUES ("
      ...
      Case "4"
        Txt.S + "ReadingDate, Operator, Vol1, Vol2, Vol4, Vol5, AniCAM_Config, Depth, Usage, HistTopSnapImage) VALUES ("
      ...
      Case "5"
        Txt.S + "ReadingDate, Operator, Vol1, Vol2, Vol3, Vol4, Vol5, AniCAM_Config, Depth, Usage, HistTopSnapImage) VALUES ("
    ...
    EndSelect
    Txt.s + ", '"+AMS_Import\AniCAM_Config+"', "+AMS_Import\Depth+", "+StrF(AMS_Import\Usage)+", ?"+")"+";"
    Database_Update(#Databases_Master,Txt.S,#PB_Compiler_Line)
...
Procedure Init_Window_Readings_Edit(RollID.i, EditType.i = 0, DataType.i = 0, ReadingID.i=-1) ;/ Manages the manual creation & editting of readings
  ...
  If Exit = 2 ;/ Create / Edit record
    ...
    Usage = Val(GetGadgetTextMac(#Gad_Reading_Usage))  
          ...
          Case #Settings_GeneralHistoryList_New
            ...
            If ResultS <> ""
              ...
              Resulti = Database_CheckForDuplicate("Select Name from AMS_General_History_Types",ResultS) ;/DNT
                                                                                                      ;/ if it doesn't exist
              If Resulti = 0
                Database_Update(#Databases_Master,"INSERT INTO AMS_General_History_Types (RollType, Name) VALUES ('1', '"+ResultS+"')",#PB_Compiler_Line) ;/DNT
              EndIf
              Refresh_GeneralHistory_List()
            EndIf
...
Procedure Init_About(parent)
  ...
  Txt + "Copyright "+Chr(169)+" 2008 - 2019 Troika Systems" + nl + nl ;/DNT
...
Procedure Init_Window_RollInformation()
  ...
  ListIconGadget(#Gad_RollInfo_Original_List,X + 60,Y-4,610,48,tTxt(#Str_Date)+":",66,#PB_ListIcon_FullRowSelect|#PB_ListIcon_GridLines|#LVS_NOCOLUMNHEADER)
  ...
  AddGadgetColumn(#Gad_RollInfo_Original_List,12,tTxt(#Str_Usage),46)
  ...
  ListIconGadget(#Gad_RollInfo_History_List,X + 60,Y,610,100,tTxt(#Str_Date),66,#PB_ListIcon_FullRowSelect|#PB_ListIcon_GridLines|#LVS_NOCOLUMNHEADER|#PB_ListIcon_AlwaysShowSelection)
  ...
  AddGadgetColumn(#Gad_RollInfo_History_List,12,tTxt(#Str_Usage),46)
  ...
  ButtonGadget(#Gad_RollInfo_Import_AMS,X + 500,Y,80,20,tTxt(#Str_Import)+Tmp) 
...
Procedure Init_Window_Main()
  ...
  ComboBoxGadget(#Gad_Graph_Type_Combo,380,2,200,22)
...
Procedure Database_Add_Usage(Roll_ID.i, SubID.i, Type)
  Protected Value.s, Current_Value.i, Older_Value.i, Newer_Value.i, OlderID.i, NewerID, TimeStamp.i, OlderEntries.i, NewerEntries.i
  
  Select Type
    Case #Reading_Master ;/ on master data
      Current_Value = Database_IntQuery("Select Usage from AMS_Roll_Master where ID = "+Str(Roll_ID)+";",#Databases_Master,#PB_Compiler_Line)
      
      Value = InputRequesterExNum("Enter Usage data","Please enter usage information:",Current_Value)
      If Value <> "" And Val(Value) => 0
        Database_Update(#Databases_Master,"UPDATE AMS_Roll_Master SET Usage='"+Value+"' where ID = "+Str(Roll_ID)+";",#PB_Compiler_Line)
      EndIf
      
    Case #Reading_Historical
      
      ;/ obtain the previous value and check to ensure that the newer value is greater.
      
      ;/ get the records timestamp
      TimeStamp = Database_IntQuery("Select ReadingDate from AMS_Roll_Data where ID = "+Str(SubID)+";",#Databases_Master,#PB_Compiler_Line)
      
      Debug "********* RollID : " + Str(Roll_ID)
      Debug "********* Roll TimeStamp: " + Str(TimeStamp)
      
      ;/ are there older values?
      OlderEntries =  Database_CountQuery("Select Count(*) from AMS_Roll_Data where RollID = "+Str(Roll_ID) +" and ReadingDate < "+Str(TimeStamp)+";",#PB_Compiler_Line)
      NewerEntries =  Database_CountQuery("Select Count(*) from AMS_Roll_Data where RollID = "+Str(Roll_ID) +" and ReadingDate > "+Str(TimeStamp)+";",#PB_Compiler_Line)
      
      Debug "********* There are "+Str(OlderEntries)+" older entries"
      Debug "********* There are "+Str(NewerEntries)+" newer entries"
      
      If OlderEntries > 0;/ get it
        ;OlderID = DatabaseQuery(#Databases_Master,"Select ID from AMS_Roll_Data where RollID = '"+Str(Roll_ID)+ " and ReadingDate < "+Str(TimeStamp)+" Order by Readingdate Desc")
        OlderID = Database_IntQuery("Select ID from AMS_Roll_Data where RollID = "+Str(Roll_ID)+ " and ReadingDate < "+Str(TimeStamp)+" Order by Readingdate Desc",#Databases_Master,#PB_Compiler_Line)
        Older_Value = Database_IntQuery("Select Usage from AMS_Roll_Data where ID = "+Str(OlderID)+";",#Databases_Master,#PB_Compiler_Line)
      Else ;/ else, get the master data entry
        Older_Value = Database_IntQuery("Select Usage from AMS_Roll_Master where ID = "+Str(Roll_ID)+";",#Databases_Master,#PB_Compiler_Line)
      EndIf
      
      If NewerEntries > 0;/ get it
        NewerID = Database_IntQuery("Select ID from AMS_Roll_Data where RollID = "+Str(Roll_ID)+ " and ReadingDate > "+Str(TimeStamp)+" Order by Readingdate Asc",#Databases_Master,#PB_Compiler_Line)
        Newer_Value = Database_IntQuery("Select Usage from AMS_Roll_Data where ID = "+Str(NewerID)+";",#Databases_Master,#PB_Compiler_Line)
      EndIf
      
      Current_Value = Database_IntQuery("Select Usage from AMS_Roll_Data where ID = "+Str(SubID)+";",#Databases_Master,#PB_Compiler_Line)
      
      Debug "********* Current ID "+Str(SubID)
      Debug "********* Older ID "+Str(OlderID)
      Debug "********* Newer ID "+Str(NewerID)
      
      Debug "********* Current Value "+Str(Current_Value)
      Debug "********* Older Value "+Str(Older_Value)
      Debug "********* Newer Value "+Str(Newer_Value)

      Value = InputRequesterExNum("Enter Usage Data","Please enter usage information:",Current_Value)
      If Value <> "" And Val(Value) => 0
        Database_Update(#Databases_Master,"UPDATE AMS_Roll_Data SET Usage='"+Value+"' where ID = "+Str(SubID)+";",#PB_Compiler_Line)
      EndIf
      
  EndSelect
  
    System\Showing_RollID = 999999
  Redraw_RollID(Roll_ID)
...
; CursorPosition = 7644
; FirstLine = 2935
; Folding = ZIAAAEAABAAAAAEAAAAAAJAACAQAUgASAYS1QAw
; Markers = 2894
; EnableXP
; EnableUser
; DPIAware
; UseIcon = Images\AniCAM_Mini_T.ico
; Executable = Executable\Anilox Management System v1.63.exe
; DisableDebugger
; Warnings = Display
; EnablePurifier
