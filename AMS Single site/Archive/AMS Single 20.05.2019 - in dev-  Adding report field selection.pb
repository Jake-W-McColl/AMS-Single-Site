...
;/ Phil James for Troika-Systems Sept 2011 - Aug 2014
...
Global Program_VersionI.i = 1060 ;/ 
;/ ************************************************************************************
...
If Multi_Site_Mode = 0
  Program_Version.s = " V1.6 - Single-Site " ;/DNT
Else
  Program_Version.s = " V1.6 - Multi-Site " ;/DNT
EndIf
...
If FileSize("RMchart.dll") < 0
  MessageRequester("Error","RMChart.dll is missing, exiting"+Chr(13),#MB_ICONWARNING) ; *MSGText=0 If this parameter is NULL,the default title Error is used.
  End
EndIf
#RMChart_Include = 1

XIncludeFile "Includes\RMChart.pbi" ;/DNT
...
;  #Gad_Reading_Quantity_Combo
  ...
  #Gad_Presets_FieldSelection_Frame
  #Gad_Presets_FieldSelection
  #Gad_Presets_Add
  #Gad_Presets_AddGeneralHistory
  #Gad_Presets_FieldList
  #Gad_Presets_FieldList_OrderUp
  #Gad_Presets_FieldList_OrderDown
  #Gad_Presets_FieldList_Remove
...
;/ Global declarations.... 
Global DatabaseFile.S, LocalSettingsDB.s, LanguagesDirectory.s, Event.i, gad.i, X.i, Y.i, *ImMem, SQL.s, DragTxt.s, DefaultComboBoxCallback.i
Global WinInfo.WinScale, System.SystemVariables, RollInfo.RollInformation, AMS_Import.AMSFile
...
System\Graph_File = GetTemporaryDirectory()+"Graph.png"
...
Procedure Database_CheckVersion(Quiet = #True)
  ...
  If Version < 11
    Database_Update(#Databases_Master,"ALTER TABLE AMS_ReportPresets ADD FieldSelection Char",#PB_Compiler_Line) ;/DNT
    AddedText + "Version 11: Added additional report field for Report system expansion." + Chr(10)               ;/DNT
    NewVersion = 11
  EndIf
...
Procedure Redraw_Graph(RollID.i)
  Debug "Graphing RollID: "+Str(RollID)
  Protected MyLoop.i, ChartID.i = 987654
  Protected CylinderNames.s, GroupName.s, GraphType.i, VolTemp.f, Vol1.f, Vol2.f, Vol3.f, Vol4.f, Vol5.f, VolCount.i, Vol_Ref.d, DateString.s, Seperator.s
  ...
  Dim Xaxis.d(1000)
  Dim VolumeAxis.d(1000)
  Dim VolumeTolerance.d(3)
  Dim MinMaxToleranceLine.d(2)
  ...
  While NextDatabaseRow(#Databases_Master)
    ...
    If GraphType = #GraphType_Volume
      ...
      DateString + UnitConversion_Date(GetDatabaseLong(#Databases_Master,5), 1) + Seperator
;      DateString + GetDatabaseInt(#Databases_Master,5)+"|"
    ...
    Else ;/ depth
      ...
      DateString + UnitConversion_Date(GetDatabaseLong(#Databases_Master,1), 1) + Seperator
    EndIf

    VolumeAxis(MyLoop) = VolTemp
    
    If MyLoop = 0
      Vol_Ref = VolTemp
    EndIf ;/ store the reference volume for adding tolerance lines
    
    Xaxis(MyLoop) = MyLoop + 1
    Debug "Volume Reading entries: "+Str(MyLoop)+" = "+StrF(VolumeAxis(MyLoop))+" - Distance#: "+Str(Xaxis(MyLoop))
  ...
  If RMC_CreateChart(GadgetID(#Gad_Graph_Image), ChartID, 0, 0, GadgetWidth(#Gad_Graph_Image)-3, GadgetHeight(#Gad_Graph_Image)-3,0,0,1) = #RMC_NO_ERROR
    ...
    RMC_AddCaption(ChartID,#ID_REGION,tTxt(#STR_Roll)+": "+GetGadgetText(#Gad_Rollinfo_RollID_String),0,0,13,#True) ; add a Caption to region 1
    ...
    If GraphType = #GraphType_Volume
      ...
      RMC_AddXYSeries(ChartID, #ID_REGION, @XAxis(0), 2, @MinMaxToleranceLine(0), 2, RGB(255,128,0), #RMC_XY_LINE, #RMC_LSTYLE_Line ,0,0,0,0)
      ;/ Red Line
      MinMaxToleranceLine(0) = Vol_Ref * (System\Settings_Capacity_Bad / 100.0)
      MinMaxToleranceLine(1) = MinMaxToleranceLine(0)
      
      XAxis(1) = XAxis(MyLoop)
      RMC_AddXYSeries(ChartID, #ID_REGION, @XAxis(0), 2, @MinMaxToleranceLine(0), 2, RGB(255,0,0), #RMC_XY_LINE, #RMC_LSTYLE_Line,0,0,0,0)
...
Procedure Report_Generate_New(SiteID.i,GroupID.i)
  Protected FilterI.i, Filter.s, MyLoop.i, Sql.s, SiteName.s, GroupCount.i, RollCount.i, LastGroup.s = "fehwiqvfhb"
  Protected Sort.i, Txt.s, Header.s = "", HasGHRecords.i
  ...
  System\Report_Type.i = GetGadgetState(#Gad_Report_Style_Combo) ;/ 0 = grouped, 1 = flat
  ...
  For MyLoop = 1 To Report_PresetList()\Fieldcount
    If Report_PresetList()\FieldBreakdown[myLoop]\Value = 99 
      SQL = "SELECT RollID, Comments FROM AMS_General_History s1 WHERE Date = (SELECT MAX(Date) FROM AMS_General_History s2 WHERE Type = '"+Report_PresetList()\FieldBreakdown[myLoop]\GHString+"' and s1.RollID = s2.RollID);"
      DatabaseQuery(#Database_Reporting, SQL)
      While NextDatabaseRow(#Database_Reporting)
        Txt.s = GetDatabaseString(#Database_Reporting, 0)+Report_PresetList()\FieldBreakdown[myLoop]\GHString
  ...
  Debug "ReportData Entries: "+Str(ListSize(ReportData()))
  
  DisableWindow(#Window_Main,1)
  OpenWindow(#Window_TestReport,0,0,640,320,"Report Testing",#PB_Window_ScreenCentered|#PB_Window_SystemMenu)
  SetGadgetFont(#PB_Default,FontID(#Font_List_S))
  
  For MyLoop = 1 To Report_PresetList()\Fieldcount
    AddElement(Report_Headers())
    Select Report_PresetList()\FieldBreakdown[myLoop]\Value
      ...
      Case #ReportField_GeneralHistory
        Report_Headers() = "GH: ["+Report_PresetList()\FieldBreakdown[myLoop]\GHString+"}"
    EndSelect
  Next
  ...
  ListIconGadget(#Gad_Report_List_Test,0,0,1024,320,"",60,#PB_ListIcon_FullRowSelect|#PB_ListIcon_GridLines|#LVS_NOCOLUMNHEADER)
  
  SendMessage_(GadgetID(#Gad_Report_List_Test),#WM_SETREDRAW,#False,0)
  
  ForEach Report_Headers()
    If ListIndex(Report_Headers()) > 0
      AddGadgetColumn(#Gad_Report_List_Test,ListIndex(Report_Headers()),"",60)
    EndIf
  Next
  
  ;/ add main header
  AddGadgetItem(#Gad_Report_List_Test,-1,tTxt(#Str_Site)+":"+Chr(10)+SiteName) : AddElement(ReportLine_RollID())
  SetGadgetItemColor(#Gad_Report_List_Test,CountGadgetItems(#Gad_Report_List_Test)-1,#PB_Gadget_BackColor,RGB(200,190,220),-1)
  AddGadgetItem(#Gad_Report_List_Test,-1,tTxt(#Str_RollCount)+":"+Chr(10)+Str(RollCount)) : AddElement(ReportLine_RollID())
  SetGadgetItemColor(#Gad_Report_List_Test,CountGadgetItems(#Gad_Report_List_Test)-1,#PB_Gadget_BackColor,RGB(220,210,240),-1)
  AddGadgetItem(#Gad_Report_List_Test,-1,tTxt(#Str_GroupCount)+":"+Chr(10)+Str(GroupCount)) : AddElement(ReportLine_RollID())
  SetGadgetItemColor(#Gad_Report_List_Test,CountGadgetItems(#Gad_Report_List_Test)-1,#PB_Gadget_BackColor,RGB(220,210,240),-1)
  ;AddGadgetItem(#Gad_Report_List_Test,-1,"") : AddElement(ReportLine_RollID())
  
  CloseDatabase(#Database_Reporting)
  
  ;/ sort report
  Select GetGadgetState(#Gad_Report_SortDirection_Combo)
    Case 0 ;/ ascending
      Sort = #PB_Sort_Ascending
    Case 1 ;/ descending
      Sort = #PB_Sort_Descending
  EndSelect
  
  Select GetGadgetState(#Gad_Report_SortBy_Combo)
    Case #Report_Sort_RollID
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\RollName),#PB_String)
    Case #Report_Sort_Capacity
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\Capacity),#PB_Integer)
    Case #Report_Sort_Manufacturer
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\Manufacturer),#PB_String)
    Case #Report_Sort_ReadingDate
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\LastProfiled),#PB_Integer)
    Case #Report_Sort_Screen
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\Screen),#PB_Integer)
    Case #Report_Sort_Variance
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\Variance),#PB_Integer)
    Case #Report_Sort_Volume
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\Volume),#PB_Float)
    Case #Report_Sort_Suitability
      SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\Suitability),#PB_String)
  EndSelect
  
  If System\Report_Type = 1 ;/ flat?
    SortStructuredList(ReportData(),Sort,OffsetOf(ReportData\Group),#PB_String)
    AddGadgetItem(#Gad_Report_List_Test,-1,Header) : AddElement(ReportLine_RollID())
    SetGadgetItemColor(#Gad_Report_List_Test,CountGadgetItems(#Gad_Report_List_Test)-1,#PB_Gadget_BackColor,RGB(250,240,150),-1)
  EndIf
  ...
  ForEach ReportData()
    ...
    If System\Report_Type = 0 ;(Grouped)
      If ReportData()\Group <> LastGroup
        ...
        AddGadgetItem(#Gad_Report_List_Test,-1,tTxt(#Str_Groupname)+":"+Chr(10)+ReportData()\Group)
        SetGadgetItemColor(#Gad_Report_List_Test,CountGadgetItems(#Gad_Report_List_Test)-1,#PB_Gadget_BackColor,RGB(220,210,210),-1)
        ...
        AddGadgetItem(#Gad_Report_List_Test,-1,tTxt(#Str_Rollcount)+":"+Chr(10)+Str(RollCount))
        SetGadgetItemColor(#Gad_Report_List_Test,CountGadgetItems(#Gad_Report_List_Test)-1,#PB_Gadget_BackColor,RGB(240,230,230),-1)
        AddElement(ReportLine_RollID())
    ...
    For MyLoop = 1 To Report_PresetList()\Fieldcount
      ...
      Select Report_PresetList()\FieldBreakdown[myLoop]\Value
        ...
        Case #ReportField_GeneralHistory
          ;Txt + ReportData()\GeneralHistory1 + Chr(10)
          Debug "GH Lookup on report - From: "+ Str(ReportData()\ID)+Report_PresetList()\FieldBreakdown[myLoop]\GHString
          Txt + GeneralHistoryLookup(Str(ReportData()\ID)+Report_PresetList()\FieldBreakdown[myLoop]\GHString)
          
      EndSelect
    Next   
    AddGadgetItem(#Gad_Report_List_Test,-1,txt)
    SetGadgetItemColor(#Gad_Report_List_Test,CountGadgetItems(#Gad_Report_List_Test)-1,#PB_Gadget_BackColor,RGB(255,255,200),-1)
  ...
  For MyLoop = 0 To Report_PresetList()\Fieldcount-1
    SendMessage_(GadgetID(#Gad_Report_List_Test), #LVM_SETCOLUMNWIDTH,MyLoop,#LVSCW_AUTOSIZE)
  Next
  
  SendMessage_(GadgetID(#Gad_Report_List_Test),#WM_SETREDRAW,#True,0)
  Repeat
    Event = WaitWindowEvent()
  Until Event = #PB_Event_CloseWindow
  
  CloseWindow(#Window_TestReport)
...
Procedure Report_Generate(SiteID.l,GroupID.l) ;/ Must pass selected Group - Show all rolls assigned to that particular Group, along with Roll Statistics
  Protected GroupName.s, RollCount.i, ResultS.s, Length.i, VarAmount.f, OnRow.i
  ...
  Filter = ""
  If Trim(Report_PresetList()\SQL) <> "" ;/ so not 'Show all'
    Debug "********************* Filtering0001 - SQL: "+Report_PresetList()\SQL
  ...
  If GroupID > 0 ;/ single Group selected
    ...
    Txt + Chr(10)+tTxt(#Str_Variance)+Chr(10)+tTxt(#Str_Capacity)+Chr(10)+tTxt(#Str_Suitability)+Chr(10)+tTxt(#Str_Lastprofiled)+Chr(10)+tTxt(#Str_Comment)
  ...
  Else ;/ must be Site instead
    ...
    If System\Report_Type = 0 ;/ Grouped
      ...
      ForEach Temp_Groups()
        ...
        If RollCount > 0 ;/ as we don't want to show empty groups...
...
Procedure.i Report_Add_GroupDetail_Original(GroupID.i,Filter.s = "")
  Protected GroupName.S, RollCount.i, ResultS.S, Length.i, VarAmount.f, OnRow.i
  Protected RollName.S, RollManufacturer.S, RollScreen.S, RollVolume.S,RollCapacity.S,RollVariance.S, SuitI.i, Suitability.s, DateI.i, LastReadDate.s, RollDepth.s
  Protected SiteName.S, GroupCount.i, MyLoop.i, RollID.i, TempI.i, Comment.s
  Protected Count.i, SQL.s
...
EndProcedure ;/ DatabaseQuery(0, "Select ID, Name, GroupID From AMS_Roll_Master Order by Name, Name+'0';")
Procedure Report_Generate_Original(SiteID.l,GroupID.l) ;/ Must pass selected Group - Show all rolls assigned to that particaulr Group, along with Roll Statistics
  Protected GroupName.s, RollCount.i, ResultS.s, Length.i, VarAmount.f, OnRow.i
  Protected RollName.s, RollManufacturer.s, RollScreen.s, RollVolume.s, RollCapacity.s, RollVariance.s, RollDepth.s
  Protected SiteName.s, GroupCount.i, MyLoop.i, RollID.i, TempI.i
  Protected Filter.s, FilterI.i, FirstCheck.i, SQL.s, GroupI.i
...
Procedure Refresh_Presets_List()
  Protected myLoop.i, onfield.i
  
  ;/ Populate Manufacturer list
  ClearList(Report_PresetList())
 
  If DatabaseQuery(#Databases_Master,"Select ID, Name, SQL, FieldSelection From AMS_ReportPresets") ;/ 
    While NextDatabaseRow(#Databases_Master)  
      ...
      Report_PresetList()\FieldSelection.s = GetDatabaseString(#Databases_Master,3) ; PJ20190625
      
      If Report_PresetList()\FieldSelection = ""
        Report_PresetList()\FieldSelection = "0||2||3||1||4||5||6||7||13||" ;/ default columns...
      EndIf
      
      ;/ breakdown field selection into constituent parts.
      Report_PresetList()\Fieldcount = CountString(Report_PresetList()\FieldSelection,"|") / 2
      onField = 1
      For myLoop = 1 To Report_PresetList()\Fieldcount
        Report_PresetList()\FieldBreakdown[myLoop]\Value = Val(StringField(Report_PresetList()\FieldSelection,onfield,"|")) : onField + 1
        Report_PresetList()\FieldBreakdown[myLoop]\GHString = StringField(Report_PresetList()\FieldSelection,onfield,"|") : onField + 1
      Next
    Wend 
    FinishDatabaseQuery(#Databases_Master)
...
Procedure.i Database_LoadSettings() ;/ Loads settings from database + Load suitability List
  ...
  If Multi_Site_Mode = 0
    ...
    System\Show_Depth = Database_IntQuery("Select Show_Depth from AMS_Settings;",#Databases_Master,#PB_Compiler_Line)
...
;  Count + Database_CountQuery("Select Count(*) From AMS_Roll_Data;")
  ...
  Database_Update(#Databases_Master, "INSERT INTO AMS_Roll_Master (Name, GroupID, DateMade) VALUES ('"+Result+"','"+Str(GroupID)+"', "+Str(1)+")",#PB_Compiler_Line) ;/DNT
...
Procedure.i Import_LoadAMS()
  ...
  If OpenFile(1,System\ImportPath+"Anilox Manager Import.ams") ;/ read the AMS header (single string) and close the file  ;/DNT
    ...
    If Left(Txt,1)="?" ;/ PJ 01.02.2017 - Suspect Unicode
      CloseFile(1)
      OpenFile(1,System\ImportPath+"Anilox Manager Import.ams") ;/ read the AMS header (single string) and close the file  ;/DNT
      Txt.S = ReadString(1,#PB_Unicode)
    EndIf
    CloseFile(1)
  Else
    ProcedureReturn 0
...
Procedure Init_Presets(Type.i)
  ...
  Width.i = 760 : Height.i = 350 : Editting = 0
  ...
  If type = 2 ;/ Edit
    ...
    If Report_PresetList()\Fieldcount > 0 : ClearList(Reportfields()) : EndIf
    
    For myLoop = 1 To Report_PresetList()\Fieldcount
      AddElement(Reportfields()) 
      Reportfields()\Value = Report_PresetList()\FieldBreakdown[myLoop]\Value
      Reportfields()\GHString = Report_PresetList()\FieldBreakdown[myLoop]\GHString
    Next
  ...
  FrameGadget(#Gad_Presets_FieldSelection_Frame,430,4,360,320,"Report Field Customization")
  ListViewGadget(#Gad_Presets_FieldSelection,440,24,140,250)
  
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_RollID)) ;"Roll Name")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Volume)) ;"Current Volume")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Depth));"Current Depth")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Screen));"Screen")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Variance));"Variance")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Capacity));"Capacity")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Suitability));"Suitability")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_LastProfiled));"Last Profiled")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Manufacturer));"Manufacturer")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_WallWidth));"Wall Width")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_CellOpening));"Cell Opening")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_LastReading));"Last Reading")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Group));"Group")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Comments)+": L1");"Comment Line 1")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Comments)+": L2");"Comment Line 2")
  AddGadgetItem(#Gad_Presets_FieldSelection,-1,tTxt(#Str_Comments)+": L3");"Comment Line 3")
  ButtonGadget(#Gad_Presets_Add,440,276,140,20,"Add Selected Field >>")
  ButtonGadget(#Gad_Presets_AddGeneralHistory,440,296,140,20,"Add General History Field")
  
  ListViewGadget(#Gad_Presets_FieldList,584,24,140,224)
  ButtonGadget(#Gad_Presets_FieldList_OrderUp,584,252,140,20,"Order Up")
  ButtonGadget(#Gad_Presets_FieldList_OrderDown,584,274,140,20,"Order Down")
  ButtonGadget(#Gad_Presets_FieldList_Remove,584,296,140,20,"Remove Selected")
  ...
  Repeat
    ...
    Select Presets_Event
      ...
      Case #PB_Event_Gadget
        Select EventGadget()
          Case #Gad_Presets_FieldList_Remove
            If GetGadgetState(#Gad_Presets_FieldList) > 0
              If MessageRequester("Remove Field...","Remove Field ["+GetGadgetItemText(#Gad_Presets_FieldList,GetGadgetState(#Gad_Presets_FieldList))+"], are you sure?",#PB_MessageRequester_YesNo) = #PB_MessageRequester_Yes
                SelectElement(Reportfields(),GetGadgetState(#Gad_Presets_FieldList))
                DeleteElement(Reportfields(),1)
                RedrawList = #True
              EndIf
            EndIf            
          Case #Gad_Presets_FieldList_OrderUp
            If GetGadgetState(#Gad_Presets_FieldList) > 0
              SetAfterRefresh = GetGadgetState(#Gad_Presets_FieldList)-1
              SelectElement(Reportfields(),GetGadgetState(#Gad_Presets_FieldList)) : *el1 = @Reportfields()
              SelectElement(Reportfields(),GetGadgetState(#Gad_Presets_FieldList)-1) : *el2 = @Reportfields()
              SwapElements(Reportfields(),*el1,*el2)
              RedrawList = #True
            EndIf
            
          Case #Gad_Presets_FieldList_OrderDown
            If GetGadgetState(#Gad_Presets_FieldList) < ListSize(Reportfields())-1
              SetAfterRefresh = GetGadgetState(#Gad_Presets_FieldList)+1
              SelectElement(Reportfields(),GetGadgetState(#Gad_Presets_FieldList)) : *el1 = @Reportfields()
              SelectElement(Reportfields(),GetGadgetState(#Gad_Presets_FieldList)+1) : *el2 = @Reportfields()
              SwapElements(Reportfields(),*el1,*el2)
          ...
          Case #Gad_Presets_FieldSelection
            If EventType() = #PB_EventType_LeftDoubleClick
              Added = #False
              ForEach Reportfields() : If Reportfields()\Value = GetGadgetState(#Gad_Presets_FieldSelection) : added = #True : EndIf  :Next
              If added = #False
              AddElement(Reportfields())
              Reportfields()\Value = GetGadgetState(#Gad_Presets_FieldSelection)
              RedrawList = #True
              EndIf
            EndIf
            
          Case #Gad_Presets_AddGeneralHistory
            Txt.s = InputRequester("Header:", "Please input Header Name:", "")
            If Txt <> ""
              AddElement(Reportfields())
          ...
          Case #Gad_Presets_Add;,440,276,140,20,"Add Selected Field >>")
            Added = #False
            ForEach Reportfields() : If Reportfields()\Value = GetGadgetState(#Gad_Presets_FieldSelection) : added = #True : EndIf  :Next
            If added = #False
              AddElement(Reportfields())
  ...
  If RedrawList = #True
    RedrawList = #False
    ClearGadgetItems(#Gad_Presets_FieldList)
    
    ForEach Reportfields()
      Select Reportfields()\Value
        Case #ReportField_RollName
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_RollID))
        Case #ReportField_CurrentVolume
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Volume))
        Case #ReportField_CurrentDepth
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Depth))
        Case #ReportField_Screen
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Screen))
        Case #ReportField_Variance
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Variance))
        Case #ReportField_Capacity
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Capacity))
        Case #ReportField_Suitability
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Suitability))
        Case #ReportField_LastProfiled
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_LastProfiled))
        Case #ReportField_Angle
          AddGadgetItem(#Gad_Presets_FieldList,-1,"Angle *Not defined*")
        Case #ReportField_Manufacturer
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Manufacturer))
        Case #ReportField_WallWidth
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_WallWidth))
        Case #ReportField_CellOpening
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_CellOpening))
        Case #ReportField_Group
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Group))
        Case #ReportField_CommentLine1
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Comments)+": L1")
        Case #ReportField_CommentLine2
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Comments)+": L2")
        Case #ReportField_CommentLine3
          AddGadgetItem(#Gad_Presets_FieldList,-1,tTxt(#Str_Comments)+": L3")
        Case #ReportField_GeneralHistory
          AddGadgetItem(#Gad_Presets_FieldList,-1,"General Hist ["+Reportfields()\GHString+"]")
      EndSelect
      
    Next
    If SetAfterRefresh > -1
      SetGadgetState(#Gad_Presets_FieldList,SetAfterRefresh)    
      SetAfterRefresh = -1
    EndIf
  ...
  If Exit = 2
    ...
    Else
      ;/ Is new entry
      Database_Update(#Databases_Master,"Insert into AMS_ReportPresets (Name, SQL, FieldSelection) VALUES ('"+GetGadgetText(#Gad_Presets_Name)+"', '"+GetGadgetText(#Gad_Presets_Query)+"','"+Txt+"')",#PB_Compiler_Line)
      Refresh_Presets_List()
    EndIf
  EndIf
...
Procedure Init_About(parent)
  ...
  Txt + "Copyright "+Chr(169)+" 2008 - 2018 Troika Systems" + nl + nl ;/DNT
...
;    SendMessage_(GadgetID(#Gad_RollInfo_GeneralHistory_History_List), #LVM_SETEXTENDEDLISTVIEWSTYLE, #LVS_EX_FULLROWSELECT, #LVS_EX_FULLROWSELECT)
    ...
    ButtonImageGadget(#Gad_RollInfo_GraphIcon, x + 654,Y,42+8,32+8,ImageID(#Image_GraphIcon))
...
Procedure Init_Window_Reporting()
  ...
  ButtonGadget(#Gad_Report_Test,x+640,y,50,20,"Test")
...
Procedure Init_Window_Main()
  ...
  ;/ Graph ********************************************************** Graph
  TextGadget(#Gad_Graph_Type_Text,300,4,80,20,tTxt(#STR_Graphingby)+":")
  SetGadgetColor(#Gad_Graph_Type_Text,#PB_Gadget_BackColor,#WinCol_Graph)
  ComboBoxGadget(#Gad_Graph_Type_Combo,380,2,100,20)
  AddGadgetItem(#Gad_Graph_Type_Combo,-1,tTxt(#STR_Volume))
  AddGadgetItem(#Gad_Graph_Type_Combo,-1,tTxt(#STR_Depth))
...
;              SetGadgetState(#Gad_Icon_Manager_Mode,0)
        ...
        Case #Gad_Report_Test
          Report_Generate_New(System\Last_Drawn_Report_SiteID,System\Last_Drawn_Report_GroupId)
...
; CursorPosition = 4456
; FirstLine = 1478
; Folding = QAAAAAAAgAqnf5hACAA1+BEKWDhztRAAoPQu+
; EnableXP
; EnableUser
; DPIAware
; UseIcon = Images\AniCAM_Mini_T.ico
; Executable = Executable\Anilox Management System v1.5h.exe
; Warnings = Display
; EnablePurifier
