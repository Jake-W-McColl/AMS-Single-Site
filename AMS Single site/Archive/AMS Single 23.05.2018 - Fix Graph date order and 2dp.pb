...
;/ Version v1.5c - 14.05.2018
;/ More DPI Scaling fixes (for Image capture routine).

;/ V1.5d - 23/05/2018 - Driven by Diflex, USA.
;/ Added Print option for Graph display
;/ Added 2 decimal points display in Graph scale when in BCM mode
;/ Added 2 decimal points display to Roll info drawing when in BCM mode & vol < 10 - Anilox QC requires fix for 2 decimal places output when in BCM mode.
;/ Added timestamp addition to date storage for better sorting.
...
If Multi_Site_Mode = 0
  Program_Version.s = " V1.5d - Single-Site " ;/DNT
Else
  Program_Version.s = " V1.5d - Multi-Site " ;/DNT
EndIf
...
Procedure.i Import_LoadAMS()
  ...
  With AMS_Import ;/ split the string into its component parts (tab delimited)
    ...
    ;/ PJ: 23052018 Added hour/minute to DB timestamp for better ordering
    \Time.s = StringField(\DateStamp,2," ") ;/ 6 -?? Sometimes '1'
    Debug "******************************************************"
...
Procedure PDF_Header()
  pdf_ImageMem("Troika AMS.png",System\OEM_ImageBuffer,System\OEM_ImageBufferSize,10,4);,55,20)  ;/DNT
...
Procedure Export_Graph_Print()
  ;Protected MyLoop.i, File.s, PageWidth.i, PageHeight.i, MinX.i, MaxX.i, MinY.i, MaxY.i, SizeX.f, SizeY.f
  Protected PageSizeX.i, PageSizeY.i, ScaleX.f, ScaleY.f, MinScale.f, Scale.f
  ;Protected GadSizeX.f, GadSizeY.f, Excluded.i, Edge_Left.i, Edge_Right.i
  ;Protected Text.s, Column.i, ColumnCount.i, SM.s, TmpTxt.s, Temp.i, Width.i, Colour.i, Pattern.s
  ;Protected NeedNewPage.i, OnNewPage.i, TemporaryImage.i, ColumnX.i, ColumnY.i, OnPage.i
  ...
  If PrintRequester()
    If StartPrinting("AMS Output - Graph")
      ...
      Scale.f = PageSizeX / ImageWidth(#Image_FullScreen)
      If PageSizeY / ImageHeight(#Image_FullScreen) < Scale
        Scale = PageSizeY / ImageHeight(#Image_FullScreen)
      EndIf
      
      ;ScaleX = ImageWidth(#Image_FullScreen) * (PageSizeX / ImageWidth(#Image_FullScreen))
      ;ScaleY = ImageHeight(#Image_FullScreen) * (PageSizeY / ImageHeight(#Image_FullScreen))
            
      If StartDrawing(PrinterOutput())
          DrawImage(ImageID(#Image_FullScreen),0,0,ImageWidth(#Image_FullScreen) * Scale,ImageHeight(#Image_FullScreen) * Scale)
        StopDrawing()
      EndIf
      StopPrinting()
    EndIf
...
;  CatchImage(#Image_Info,?Info)
  ...
  Define Scl.i
  Debug "Image Width: "+Str(ImageWidth(#Image_OEM_Logo))+" - " + "Image Width: "+Str(ImageHeight(#Image_OEM_Logo))
  If ImageWidth(#Image_OEM_Logo) / 260.0 > ImageHeight(#Image_OEM_Logo) / 94.0
    Debug "a: " + 
    ResizeImage(#Image_OEM_Logo,ImageWidth(#Image_OEM_Logo) / (ImageWidth(#Image_OEM_Logo) / 260.0),ImageHeight(#Image_OEM_Logo) / (ImageWidth(#Image_OEM_Logo) / 260.0))
  Else
    Debug "b"
    ResizeImage(#Image_OEM_Logo,ImageWidth(#Image_OEM_Logo) / (ImageHeight(#Image_OEM_Logo) / 94.0),ImageHeight(#Image_OEM_Logo) / (ImageHeight(#Image_OEM_Logo) / 94.0))
  EndIf
...
;   If FileSize("OEM\OEM Logo PDF.png") > 0
;     System\OEMLogo.s = "OEM\OEM Logo PDF.png" ;/ have to ensure the PDF image doesn't have any alpha channel (saving as JPG does that).
;     SaveImage(#Image_OEM_Logo,"OEM\OEM Logo PDF.jpg",#PB_ImagePlugin_JPEG,10,24)
;     LoadImage(#Image_OEM_Logo_Opaque,"OEM\OEM Logo PDF.jpg")
;   EndIf
...
; CursorPosition = 688
; FirstLine = 683
; Folding = TbAANIAAIAAAwAACIyQKAIAAEHCAKCOUYg5
; EnableXP
; EnableUser
; UseIcon = Images\AniCAM_Mini_T.ico
; Executable = Executable\Anilox Management System v1.5d.exe
; Warnings = Display
; EnablePurifier
