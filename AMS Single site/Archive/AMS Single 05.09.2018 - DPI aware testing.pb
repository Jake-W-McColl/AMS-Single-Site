...
If Multi_Site_Mode = 0
  Program_Version.s = " V1.5e - Single-Site " ;/DNT
Else
  Program_Version.s = " V1.5e - Multi-Site " ;/DNT
EndIf
...
Enumeration ;/Images
  ...
  ;/ icon images (no scaling for DPI
  #Image_TreeView_Home
...
Global event.i, gad.i, X.i, Y.i, *ImMem, SQL.s, DragTxt.s, DefaultComboBoxCallback.i
...
Procedure ResizeImage_(Image,Width,Height, Mode = 0)
  CompilerIf #PB_Compiler_Version < 570
    If Not IsImage(Image) : ProcedureReturn : EndIf
    If ImageFormat(Image) = #PB_ImagePlugin_ICON : ProcedureReturn : EndIf
    ResizeImage(Image,Width,Height,mode)
  CompilerElse
    If Not IsImage(Image) : ProcedureReturn : EndIf
    If ImageFormat(Image) = #PB_ImagePlugin_ICON : ProcedureReturn : EndIf
    ResizeImage(Image,DesktopScaleX(Width),DesktopScaleY(Height),mode)
  CompilerEndIf
EndProcedure

Procedure CatchImage_(Image,*mem,size.i=0)
  CatchImage(Image,*mem)
  CompilerIf #PB_Compiler_Version >= 570
    ResizeImage_(Image,ImageWidth(Image),ImageHeight(Image))
  CompilerEndIf
EndProcedure

Procedure LoadImage_(Image,file.s,flags = 0)
  If Image = #PB_Any
    Image = LoadImage(Image, File.s, flags)
  Else
    LoadImage(Image, File.s, flags)
  EndIf
  CompilerIf #PB_Compiler_Version >= 570
    ResizeImage_(Image,ImageWidth(Image),ImageHeight(Image))
  CompilerEndIf
...
Procedure Database_BitmapToJPG() ;/ database version 10 requirement
  ...
  ForEach BlobList()
    CatchImage_(1,BlobList()\Mem,BlobList()\Size)
...
Procedure DisplaySplash(ImageID,time.i)
  ...
  CompilerIf #PB_Compiler_Version >= 570
    OpenWindow(#Window_Splash,0,0,DesktopUnscaleX(ImageWidth(ImageID)),DesktopUnscaleY(ImageHeight(ImageID)),"",#PB_Window_BorderLess|#PB_Window_ScreenCentered|#PB_Window_Invisible) ;/DNT
  CompilerElse
...
Procedure.i Image_To_Memory(File.S) ;/ 
  ...
  *ImMem = AllocateMemory(Size)
  ...
  Image = LoadImage_(#PB_Any,File.s)
...
;    EndIf
;    
;    ResizeImage_(Image,ImageWidth(Image)*Rescale,ImageHeight(Image)*Rescale))
...
Procedure Form_Update_Images()
  ...
  If IsImage(#Image_BorderRolls) 
      ...
      ResizeImage_(#Image_BorderRolls_Scaled,Wid,Hgt)
  ...
  If IsImage(#Image_2d_Image_Current)
    ...
    ResizeImage_(#Image_RollInfo_Current_Scaled,GadgetWidth(#Gad_RollInfo_Image_Latest)-4,GadgetHeight(#Gad_RollInfo_Image_Latest)-4)
  ...
  If IsImage(#Image_2d_Image_Reference)
    ...
    ResizeImage_(#Image_RollInfo_Referencel_Scaled,GadgetWidth(#Gad_RollInfo_Image_Reference)-4,GadgetHeight(#Gad_RollInfo_Image_Reference)-4)
  ...
  If IsImage(#Image_Roll_Large)
    ...
    ResizeImage_(#Image_Roll_Large_Scaled,GadgetWidth(#Gad_RollInfo_Roll_Image),GadgetHeight(#Gad_RollInfo_Roll_Image))
  ...
  If IsImage(#Image_Roll_Pins)
    ...
    ResizeImage_(#Image_Roll_Pins_Scaled,GadgetWidth(#Gad_RollInfo_Roll_Pins),GadgetHeight(#Gad_RollInfo_Roll_Pins))
  ...
  If IsImage(#Image_Logo)
    ...
    ResizeImage_(#Image_Logo_Scaled,GadgetWidth(#Gad_TroikaButton),GadgetHeight(#Gad_TroikaButton))
  ...
  If IsImage(#Image_TroikaAMS)
    ...
    ResizeImage_(#Image_TroikaAMS_Scaled,GadgetWidth(#Gad_Welcome_TroikaLogo),GadgetHeight(#Gad_Welcome_TroikaLogo))
  ...
  If IsImage(#Image_Graph)
    ...
    ResizeImage_(#Image_Graph_Scaled,GadgetWidth(#Gad_Graph_Image),GadgetHeight(#Gad_Graph_Image))
...
Procedure Image_Refresh_History(HistoryID.i) ;/
  ...
  If Length > 0
    ...
    *ImMem = AllocateMemory(Length)
    ...
    CatchImage_(#Image_2d_Image_Current ,*ImMem,Length)
...
;  CheckDatabaseUpdate(#Databases_Master, SQL)
  ...
  If Length > 0
    ...
    CatchImage_(#Image_2d_Image_Reference,System\ReferenceImage,Length)
  ...
  If System\Selected_HistoryID > -1
    ...
    If Length > 0
      ...
      CatchImage_(#Image_2d_Image_Current ,System\CurrentImage,Length)
    EndIf 
    FinishDatabaseQuery(#Databases_Master)
  EndIf
...
;      DateString + GetDatabaseInt(#Databases_Master,5)+"|"
  ...
  If RMC_CreateChart(GadgetID(#Gad_Graph_Image), ChartID, 0, 0, GadgetWidth(#Gad_Graph_Image)-3, GadgetHeight(#Gad_Graph_Image)-3,0,0,1) = #RMC_NO_ERROR
    ...
    LoadImage_(#Image_Graph,System\Graph_File)
...
Procedure Image_Manual_Load(Master.i=0,HistoryID.i=0)
  ...
  If File
    ...
    If LCase(GetExtensionPart(File)) <> "jpg" ;/ only store compressed images
      ImportImage = LoadImage_(#PB_Any,File)
      If ImportImage <> 0
      ...
      If StartDrawing(PrinterOutput())
        ...
        For Myloop = 0 To CountGadgetItems(#Gad_Report_ReportList)
          ...
          If OnNewPage = 1
            ...
            ResizeImage_(#Image_Temp,ImageWidth(#Image_OEM_Logo_Opaque) * ImgScale,ImageHeight(#Image_OEM_Logo_Opaque) * ImgScale,#PB_Image_Raw)
  ...
  If PrintRequester()
    If StartPrinting("AMS Output - Graph")
      ...
      If StartDrawing(PrinterOutput())
          ...
          CopyImage(#Image_OEM_Logo_Opaque,#Image_Temp)
          ImgScale = (64*ScaleY) / ImageHeight(#Image_Temp) 
          ResizeImage_(#Image_Temp,ImageWidth(#Image_OEM_Logo) * ImgScale,ImageHeight(#Image_OEM_Logo) * ImgScale,#PB_Image_Raw)
...
;  url.S = StringField(Txt, 5, nf)
  ...
  If Win
    ...
    If System\Manager_Mode = 1
      Lnk2 = HyperLinkGadget(#PB_Any,70,220,220,20,System\Database_Path,RGB(0,0,250))
...
Procedure Init_FullScreen() ;/ Full screen quickviewer
  ...
  ResizeImage_(#Image_FullScreen,MaxSize,MaxSize)
...
; 
  ...
  ; StopDrawing()
; 
  ; ResizeImage_(#Image_FullScreen,1920-1,1200-1)
  ; 
...
Procedure.i Handle_2d_Analysis(View.i)
  ...
  If View = 0
    If #Image_2d_Image_Reference <> 0
      ...
      If System\Zoom_2d > 0
        ...
        ResizeImage_(#Image_2d_Analysis,636,476)
      EndIf
  ...
  If View = -1 ;/ update zoom image
    ...
    StopDrawing()
    ResizeImage_(#Image_2d_Scaled,128,128)
...
Procedure Init_Images()
  Protected Scale.f, MyLoop.i
  
  CatchImage_(#Image_Splash,?Splash); : ResizeImage_(#Image_Splash,DesktopScaleX(ImageWidth(#Image_Splash)),DesktopScaleY(ImageHeight(#Image_Splash)))
  CatchImage_(#Image_Logo,?Logo) ; : ResizeImage_(#Image_Splash,DesktopScaleX(ImageWidth(#Image_Splash)),DesktopScaleY(ImageHeight(#Image_Splash)))

  CatchImage_(#Image_Logo_Opaque,?Logo_Opaque)
  CatchImage_(#Image_TreeView_Plant,?Chart)
  CatchImage_(#Image_TreeView_Group,?Group2)
  CatchImage_(#Image_Roll,?SmallRoll)
  CatchImage_(#Image_Roll_Large,?Roll)
  CatchImage_(#Image_Roll_Pins,?Roll_Pins)
  CatchImage_(#Image_TreeView_Home,?home)
;  CatchImage_(#Image_Info,?Info)
  CatchImage_(#Image_Treeview_Factory,?Factory2)
  CatchImage_(#Image_Star,?star)
  CatchImage_(#Image_RollInfo_Opening,?Roll_Opening)
  CatchImage_(#Image_RollInfo_Screen,?Roll_Screen)
  CatchImage_(#Image_RollInfo_Wall,?Roll_Wall)
  ;  CatchImage_(#Image_RollInfo_Sample,?Sample)
  CatchImage_(#Image_AniCAM,?AniCAM)
  CatchImage_(#Image_AniCAM_Opaque,?AniCAM_Opaque)
  CatchImage_(#NoImageLoaded,?NoImageLoaded)
  CatchImage_(#Image_AniCAMIcon,?AniCAMIcon)
  
  CatchImage_(#Image_GraphIcon,?GraphIcon_Start)
  
  CatchImage_(#Image_Export_XLS,?Icon_XLS)
  CatchImage_(#Image_Export_CSV,?Icon_CSV)
  CatchImage_(#Image_Export_PDF,?Icon_PDF)
  CatchImage_(#Image_Export_IMG,?Icon_IMG)
  CatchImage_(#Image_AutoImport,?Icon_AutoImport)

  CatchImage_(#Image_RollInfo_Opening_Opaque,?Roll_Opening_Opaque)
  CatchImage_(#Image_RollInfo_Screen_Opaque,?Roll_Screen_Opaque)
  CatchImage_(#Image_RollInfo_Wall_Opaque,?Roll_Wall_Opaque)
  CatchImage_(#Image_RollInfo_Roll_Opaque,?Roll_Opaque)
  CatchImage_(#Image_RollInfo_Roll_Pins_Opaque,?Roll_Pins_Opaque)
  
  CatchImage_(#Image_BorderRolls,?BorderRolls)
  CatchImage_(#Image_TroikaAMS,?TroikaAMS)
  
  CatchImage_(#Image_Padlock_Open,?Padlock_Open)
  CatchImage_(#Image_Padlock_Closed,?Padlock_Closed)
  CatchImage_(#Image_PrinterIcon,?Print_Start)
  ...
  If FileSize("OEM\OEM Logo.bmp") > -1
    ...
    LoadImage_(#Image_OEM_Logo,"OEM\OEM Logo.bmp")
  ElseIf FileSize("OEM\OEM Logo.jpg") > -1
    Debug "************************ Loaded OEM Logo - JPG"
    LoadImage_(#Image_OEM_Logo,"OEM\OEM Logo.jpg")
  ElseIf FileSize("OEM\OEM Logo.png") > -1
    Debug "************************ Loaded OEM Logo - PNG"
    LoadImage_(#Image_OEM_Logo,"OEM\OEM Logo.png")
  EndIf
  ...
  If ImageWidth(#Image_OEM_Logo_Scaled) / 260.0 > ImageHeight(#Image_OEM_Logo_Scaled) / 94.0
    Debug "a: " + 
    ResizeImage_(#Image_OEM_Logo_Scaled,ImageWidth(#Image_OEM_Logo) / (ImageWidth(#Image_OEM_Logo) / 260.0),ImageHeight(#Image_OEM_Logo) / (ImageWidth(#Image_OEM_Logo) / 260.0))
  Else
    Debug "b"
    ResizeImage_(#Image_OEM_Logo_Scaled,ImageWidth(#Image_OEM_Logo) / (ImageHeight(#Image_OEM_Logo) / 94.0),ImageHeight(#Image_OEM_Logo) / (ImageHeight(#Image_OEM_Logo) / 94.0))
  EndIf
  ...
  CatchImage_(#Image_OEM_Logo_Opaque_Scaled,System\OEM_ImageBuffer)
  ...
  System\OEM_ImageBufferSize = MemorySize(System\OEM_ImageBuffer)
  CatchImage_(#Image_OEM_Logo_Opaque,System\OEM_ImageBuffer)
...
; CursorPosition = 5486
; FirstLine = 1684
; Folding = QAAAIAAgDBAAAGAQC6GSBABAzbQSU6WpiHE-
; EnableXP
; EnableUser
; UseIcon = Images\AniCAM_Mini_T.ico
; Executable = Executable\Anilox Management System v1.5e.exe
; Warnings = Display
; EnablePurifier
