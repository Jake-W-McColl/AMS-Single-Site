...
Procedure Database_CheckVersion(Quiet = #True)
  ...
  If Version < 20 ;/ skip a couple of versions to puh through updates on 'inbetween' releases...
    ...
    Txt.s = "DROP TRIGGER trigger_update_usage_roll_master;"
    Database_Update(#Databases_Master,txt,#PB_Compiler_Line)   
    
    Txt.s = "DROP TRIGGER trigger_update_usage_roll_data;"
    Database_Update(#Databases_Master,txt,#PB_Compiler_Line)   
...
Procedure Redraw_RollID(RollID.i, Force.i=0)
  ...
  SQL.s + "Operator, ReadingDate, Vol1,Vol2,Vol3,Vol4,Vol5,Volume, Capacity, Variance, Depth, Current_Depth, Usage, Current_Usage"
...
Procedure Redraw_Graph(RollID.i, RollName.s = "", ForExport = 0)
  ...
  If RMC_CreateChart(GadgetID(#Gad_Graph_Image), ChartID, 0, 0, GadgetWidth(#Gad_Graph_Image)-3, GadgetHeight(#Gad_Graph_Image)-3,0,0,1) = #RMC_NO_ERROR
    ...
    StartDrawing(ImageOutput(#Image_Graph))
      ...
      For y = 53 To 559
        If Point(74,Y) = 33023 : DrawText(ImageWidth(#Image_Graph)-60,Y-17,Str(System\Settings_Capacity_Good)+"%",0) : EndIf
        If Point(74,Y) = 255 : DrawText(ImageWidth(#Image_Graph)-60,Y-17,Str(System\Settings_Capacity_Bad)+"%",0) : EndIf
      Next
    StopDrawing()
...
Procedure Database_ExportRoll_Images(RollID.i)
  ...
  NewList RollMaster_Export.RollMaster_Export()
  ...
  Length = DatabaseColumnSize(#Databases_Master, 0) ; Display the content of the first field
  ;Debug "ColumnType: "+Str(DatabaseColumnType(#Databases_Master,0))+" - Length: "+Str(Length)
  ...
  DatabaseQuery(#Databases_Master,"Select ID, HistTopSnapImage where RollID = "+Str(RollID)+";")
  While NextDatabaseRow(#Databases_Master)
    Length = DatabaseColumnSize(#Databases_Master, 1) ; Display the content of the first field  
    If Length > 0
      If System\CurrentImage <> 0 : FreeMemory(System\CurrentImage) : EndIf
      System\CurrentImage = AllocateMemory(Length)
      System\Current_Image_Length = Length
      GetDatabaseBlob(#Databases_Master , 1,System\CurrentImage,Length)
      CatchImage(#Image_2d_Image_Current, System\CurrentImage,Length)
      SaveImage(#Image_2d_Image_Current,Path+"HistTopSnap"+Str(GetDatabaseLong(#Databases_Master,0))+".bmp",#PB_ImagePlugin_BMP)
    EndIf 
  Wend        
  FinishDatabaseQuery(#Databases_Master) 
  
  Redraw_Graph(RollID,RollMaster_Export()\Name)
  SaveImage(#Image_Graph,Path+"Graph.bmp",#PB_ImagePlugin_BMP)
EndProcedure
...
Procedure.s Database_BuildRollLog(RollID.i) ;- _Working on BuildRollLog
  ...
  RollMaster_Export()\GroupID = GetDatabaseString(#Databases_Master,0)
  RollMaster_Export()\Name = GetDatabaseString(#Databases_Master,1)
  RollMaster_Export()\Manufacturer = GetDatabaseString(#Databases_Master,2)
  RollMaster_Export()\Width = GetDatabaseString(#Databases_Master,3)
  RollMaster_Export()\Suitability = GetDatabaseString(#Databases_Master,4)
  ...
  RollMaster_Export()\Operator = GetDatabaseString(#Databases_Master,10)
  RollMaster_Export()\ReadingDate = GetDatabaseString(#Databases_Master,11)
  RollMaster_Export()\Vol1 = GetDatabaseString(#Databases_Master,12)
  RollMaster_Export()\Vol2 = GetDatabaseString(#Databases_Master,13)
  RollMaster_Export()\Vol3 = GetDatabaseString(#Databases_Master,14)
  RollMaster_Export()\Vol4 = GetDatabaseString(#Databases_Master,15)
  RollMaster_Export()\Vol5 = GetDatabaseString(#Databases_Master,16)
  RollMaster_Export()\Volume = GetDatabaseString(#Databases_Master,17)
  RollMaster_Export()\Capacity = GetDatabaseString(#Databases_Master,18)
  RollMaster_Export()\Variance = GetDatabaseString(#Databases_Master,19)
  RollMaster_Export()\lastreadingdate = GetDatabaseString(#Databases_Master,20)
  RollMaster_Export()\AniCAM_Config = GetDatabaseString(#Databases_Master,21)
  RollMaster_Export()\Depth = GetDatabaseString(#Databases_Master,22)
  RollMaster_Export()\Current_Depth = GetDatabaseString(#Databases_Master,23)
  RollMaster_Export()\Usage = GetDatabaseString(#Databases_Master,24)
  RollMaster_Export()\Current_Usage = GetDatabaseString(#Databases_Master,25)
  FinishDatabaseQuery(#Databases_Master)
  
  ;/ ReadingDate, Operator, Vol1, Vol2, Vol3, Vol4, Vol5, ImageName
  DatabaseQuery(#Databases_Master,"Select ID, Operator, ReadingDate, Vol1, Vol2, Vol3, Vol4, Vol5, AniCAM_Config, Depth, Usage from AMS_Roll_Data where RollID = "+Str(RollID)+";")
  While NextDatabaseRow(#Databases_Master)
    AddElement(RollMaster_Export())
    RollMaster_Export()\ID = GetDatabaseString(#Databases_Master,0)
    RollMaster_Export()\Operator = GetDatabaseString(#Databases_Master,1)
    RollMaster_Export()\ReadingDate = GetDatabaseString(#Databases_Master,2)
    RollMaster_Export()\Vol1 = GetDatabaseString(#Databases_Master,3)
    RollMaster_Export()\Vol2 = GetDatabaseString(#Databases_Master,4)
    RollMaster_Export()\Vol3 = GetDatabaseString(#Databases_Master,5)
    RollMaster_Export()\Vol4 = GetDatabaseString(#Databases_Master,6)
    RollMaster_Export()\Vol5 = GetDatabaseString(#Databases_Master,7)
    RollMaster_Export()\AniCAM_Config = GetDatabaseString(#Databases_Master,8)
    RollMaster_Export()\Depth = GetDatabaseString(#Databases_Master,9)
    RollMaster_Export()\Usage = GetDatabaseString(#Databases_Master,10)
  Wend
  FinishDatabaseQuery(#Databases_Master)
...
Procedure.i Import_AMS(NoLoad.i = 0) ;/ NoLoad for Master Import, to prevent reloading of asset
  ...
  If Date = 0 ;/ Assume no Original import, so Import to Roll Master
    ...
    If AMS_Import\Usage > 0 : Txt.S + ", Current_Usage = "+StrF(AMS_Import\Usage) : EndIf
...
Procedure Init_Window_RollInformation()
  ...
  ListIconGadget(#Gad_RollInfo_Original_List,X + 60,Y-4,618,48,tTxt(#Str_Date)+":",66,#PB_ListIcon_FullRowSelect|#PB_ListIcon_GridLines|#LVS_NOCOLUMNHEADER)
  ...
  ListIconGadget(#Gad_RollInfo_History_List,X + 60,Y,618,100,tTxt(#Str_Date),66,#PB_ListIcon_FullRowSelect|#PB_ListIcon_GridLines|#LVS_NOCOLUMNHEADER|#PB_ListIcon_AlwaysShowSelection)
...
; CursorPosition = 12499
; FirstLine = 2638
; Folding = dKCAAAeAAgAAAAACAAAACAAAAAAAAAAAAAAUIBf5x7
; Markers = 3020
; EnableThread
; EnableXP
; EnableUser
...
; Executable = Executable\Anilox Management System v1.64.exe
; Warnings = Display
; EnablePurifier
