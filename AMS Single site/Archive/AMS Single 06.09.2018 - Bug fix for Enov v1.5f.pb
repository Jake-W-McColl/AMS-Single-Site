...
If Multi_Site_Mode = 0
  Program_Version.s = " V1.5f - Single-Site " ;/DNT
Else
  Program_Version.s = " V1.5f - Multi-Site " ;/DNT
EndIf
...
Procedure Process_Window_Events()
  ...
  If EventType() = #PB_EventType_Focus And GadgetType(EventGadget())=#PB_GadgetType_String
    Debug "Sending #EM_SETSEL message..."
    SendMessage_(GadgetID(EventGadget()),#EM_SETSEL,0,-1)
    Debug "Sent #EM_SETSEL message..."
  EndIf
...
;        Debug "Does: "+Str(EventDropText())+" = 0 ?"
    ...
    Case #PB_Event_Menu 
      Debug ("Menu Item: "+Str(EventMenu()) +" selected")  ;/DNT
      Process_Menu_Events()
      
    Case #PB_Event_Gadget
      
      System\LastSelectedGadget = EventGadget()
      Debug "Gadget event: "+System\LastSelectedGadget
      Select System\LastSelectedGadget
...
Debug "Loading Settings..." : Database_LoadSettings()
Debug "Parsing Language files..." : Language_Parse_Files()
Debug "Setting Languages..." : Language_Set(System\Settings_Language)
Debug "Refreshing Suitability List..." : Refresh_Suitability_List()
Debug "Refreshing Manufacturers List..." : Refresh_Manufacturer_List()
Debug "Checking Database version..." : Database_CheckVersion() ;/ checks current database version & updates if required
Debug "Initializing PDF functions..." : PDF_Init()
Debug "Loading SQL..." : SQL_Load_Examples()
Debug "Refreshing Group List..." : Refresh_Group_List(1)
Debug "Init - Begin..." : Init_Begin()
Debug "Init Main window..." : Init_Window_Main()
Debug "Init End..." : Init_End()
Debug "Redraw NavTree..." : Redraw_NavTree()
Debug "Displaying Window..." : Show_Window(#Panel_HomeScreen,#PB_Compiler_Line)
Debug "Dynamic Control resize..." : ResizeGadgets()

;{- Main loop
System\LiveMonitorNextTime = ElapsedMilliseconds() + 2000
Repeat
  System\Window_Main_Events = WaitWindowEvent(500) 
  Debug "Main loop (500ms delay)"
  If System\Window_Main_Events <> 0
    Debug "Processing window events...."+Str(ElapsedMilliseconds())
  ...
  If GetAsyncKeyState_(#VK_F11)
    Debug "F11 pressed.... - Could halt here (waiting for F11 to be depressed)."
    Repeat : Until GetAsyncKeyState_(#VK_F11) = 0
    Init_FullScreen()
  EndIf
  ...
  If System\Refresh_Roll_Information > 0
    Debug "Refreshing Roll information"
    If GetGadgetState(#Gad_NavTree) < 0 : SetGadgetState(#Gad_NavTree,0) : EndIf 
    SelectElement(NavTree(),GetGadgetState(#Gad_NavTree))
    If NavTree()\RollID > 0
      Redraw_RollID(NavTree()\RollID)
...
; CursorPosition = 12259
; FirstLine = 5693
; Folding = RAAANQAAJAgHwAACIyYLAYAIAHSCKHSEAg5
; EnableXP
; EnableUser
; DPIAware
; UseIcon = Images\AniCAM_Mini_T.ico
; Executable = Executable\Anilox Management System v1.5f for Enov.exe
; Warnings = Display
; EnablePurifier
