...
CompilerIf #Log_Functions = #True
  OpenConsole()
CompilerEndIf
Procedure Console_Log(FunctionTxt.s, Mode) ;/ trapping freezing issue on customer pc - Mode 0 = Enter, 1 = Exit
  Protected Out.s
  Static LastOut.s
  
  Select Mode
    Case 0
      Out = "Function: "+FunctionTxt+" - Entrance"
    Case 1
      Out = "Function: "+FunctionTxt+" - Exit"
    Default
      Out = FunctionTxt
  EndSelect
...
Global IniSwitch_NoAdmin.i = 0, IniSwitch_ReportDepths.i, IniSwitch_WallsInsteadOfSuitability.i
...
Procedure.i DragCallBack(Action)
  CompilerIf #Log_Functions = #True : Console_Log("DragCallBack",0) : CompilerEndIf
  ...
  If action<>#PB_Drag_None
    ...
    ;Check if the tree gadget requires scrolling.
      ...
      EndIf
    EndIf
    CompilerIf #Log_Functions = #True : Console_Log("DragCallBack",1) : CompilerEndIf
  ProcedureReturn 1
EndProcedure
...
Procedure SetListIconColumnJustification(ListIconID, Column, Alignment)
  ...
  If #Log_Functions = #True : Console_Log("SetListIconColumnJustification",0) : EndIf
  ...
  If #Log_Functions = #True : Console_Log("SetListIconColumnJustification",1) : EndIf
EndProcedure
...
Procedure.s HardwareFingerprint()
  ...
  ;/ ***************************************************************************************************************
  CompilerIf #Log_Functions = #True : Console_Log("HardwareFingerprint",0) : CompilerEndIf
  Protected hwp.HW_PROFILE_INFO
  GetCurrentHwProfile_(@hwp)
  Debug PeekS(@hwp\szHwProfileName[0]) + " -> " + PeekS(@hwp\szHWProfileGUID[0])
  CompilerIf #Log_Functions = #True : Console_Log("HardwareFingerprint",1) : CompilerEndIf
...
Procedure Database_Backup() 
  ...
  CompilerIf #Log_Functions = #True : Console_Log("Database_Backup",0) : CompilerEndIf
  ...
  If Path <> ""
    ...
    EndIf
  EndIf
  CompilerIf #Log_Functions = #True : Console_Log("Database_Backup",1) : CompilerEndIf

EndProcedure

Procedure Database_Restore()
  Protected Path.s, FileFrom.s, FileTo.s
  CompilerIf #Log_Functions = #True : Console_Log("Database_Restore",0) : CompilerEndIf
  ...
  If Path <> ""
    ...
    EndIf
  EndIf
    CompilerIf #Log_Functions = #True : Console_Log("Database_Restore",1) : CompilerEndIf
...
Procedure Database_Update(Database, query.S,LineNum.i)
  ...
  CompilerIf #Log_Functions = #True : Console_Log("Database_Update",0) : CompilerEndIf  
  ...
  EndIf
  CompilerIf #Log_Functions = #True : Console_Log("Database_Update",1) : CompilerEndIf
...
Procedure.i Database_CountQuery(SQL.S, LineNum.i, Quiet.i = 0)
  ...
  ;/  list with the query text and the time it has taken to run.
  CompilerIf #Log_Functions = #True : Console_Log("Database_CountQuery",0) : CompilerEndIf
  ...
  CompilerIf #Log_Functions = #True : Console_Log(" Database_CountQuery",1) : CompilerEndIf
...
Procedure Apply_LicenceonLoad(File.s) ;/ PJ20190519
...
Repeat
  ...
  Console_Log("Main Loop (Event Handling)",2)
...
; CursorPosition = 711
; FirstLine = 38
; Folding = kAAAAAAAMAAAAAAAAAChAgDAAACEBAgMLck+
; EnableXP
; EnableUser
; DPIAware
; UseIcon = Images\AniCAM_Mini_T.ico
; Executable = Archive\Executable\Anilox Management System v1.6.exe
; DisableDebugger
; Warnings = Display
; EnablePurifier
