...
Procedure ipdf_Handle_MS_Encoding(Array glyph_list.TTF_CharacterMap(1),cmap_n_segs,*cmap_seg_start.TTF_Segment,*cmap_seg_end.TTF_Segment,*cmap_idDelta.TTF_Segment,*cmap_idRangeOffset.TTF_Segment,*cmap_glyphIndexArray.TTF_Segment,nglyphs)
  Protected j, k, kk, set_ok, *glyphIndexAddress.TTF_Segment
  Protected s_start.u,s_end.u,ro.u,delta.w,n.w
      ...
      Else
        ...
        *glyphIndexAddress = (ro + 2 * (k - s_start) + @*cmap_idRangeOffset\segment[j])
        If delta<>0
          ...
          CallDebugger
        EndIf
        n=ipdf_EndianW(*glyphIndexAddress\segment[0])
      EndIf
      If n<0 Or n>=nglyphs
        Debug "Font contains a broken glyph code mapping, ignored"
        Continue
...
Procedure ipdf_Handle_MAC_Encoding(Array glyph_list.TTF_CharacterMap(1),*subtable_F0.TTF_cmap_Format_0)
...
Procedure.i ipdf_TTF_Searchcmap(Array glyph_list.TTF_CharacterMap(1),*cmap.TTF_cmap_Header,nglyphs,platform=-1,encoding=-1,format=-1)
      ...
      Select formatID
        Case 0 ; Format 0: Byte encoding table
          ...
          *subtable_F0=*subtable
          ...
          ipdf_Handle_MAC_Encoding(glyph_list(),*subtable_F0)
          ProcedureReturn #True
        Case 4 ; Format 4: Segment mapping to delta values
          ...
          *ptr=*subtable+SizeOf(TTF_cmap_Format_4)
          ...
          ipdf_Handle_MS_Encoding(glyph_list(),formatF4_segCount,cmap_seg_start,cmap_seg_end,cmap_idDelta,cmap_idRangeOffset,cmap_glyphIndexArray,nglyphs)
          ProcedureReturn #True
      EndSelect
      *cmap_Table_Header+SizeOf(TTF_cmap_Header_Table)
      cmap_numTables-1
...
ProcedureDLL pdf_Init()
  Global NewList Pages.MEM_DataStructure()
  Global NewList PagesF.PDF_PageStructure()
  ...
  Global NewList OrientationChanges.l() 
...
Procedure ipdf_Out(String$) ;Add a line to the document.
  If pdfState = 2
    ...
    MEM_DataAddString(Pages(), String$ + #PDF_NEWLINE)
  Else
...
Procedure ipdf_EscapeU(First$,Last$,Text$,*MEM.MEM_DataStructure,UnicodeHeader=#False)
  Protected i,j,sLen=Len(Text$),Char.u,PChar.b,*Ptr,Unicode
  
  If UnicodeHeader=#True Or Fonts()\unicode=#True
    Unicode=#True
  EndIf
...
Procedure ipdf_ParsePNG(*aData.MEM_DataStructure)
  Protected vReturn,vCt,vCount,vNum,vType.s
...
Procedure ipdf_ParseJPG(*aData.MEM_DataStructure)  
  Protected vStart,vLength,vFound,vResult
  ...
  While (vFound <> #True) And (vStart + 9 < *aData\lCurSize)
    If PeekB(*aData\pData + vStart) = -1 
      If PeekB(*aData\pData + vStart + 1) = -64 
        vFound = #True
...
Procedure ipdf_BeginPage(Orientation$,Format$)
  pdfPage = pdfPage + 1
  AddElement(Pages())
  If MEM_DataInit(Pages(),64000) = #False
    pdfError = #PDF_ERROR_OUT_OF_MEMORY
    ProcedureReturn #False
  EndIf
  
  AddElement(OrientationChanges())
  ...
  AddElement(PagesF())
  PagesF()\pdfWPt=pdfWPt
  PagesF()\pdfHPt=pdfHPt
...
Procedure ipdf_PutCatalog()
  ...
  Select pdfZoomMode
    Case #PDF_ZOOM_FULLPAGE
      ipdf_Out("/OpenAction [1 0 R /Fit]")
    Case #PDF_ZOOM_FULLWIDTH
      ipdf_Out("/OpenAction [1 0 R /FitH null]")
    Case #PDF_ZOOM_REAL
      ipdf_Out("/OpenAction [1 0 R /XYZ null null 1]")
    Case #PDF_ZOOM_FULLSCREEN    ; <-- luis
      ipdf_Out("/PageMode/FullScreen")  ; <-- luis
    Default:
      ipdf_Out("/OpenAction [1 0 R /XYZ null null " + ipdf_StrF(pdfZoomMode/100,2) +  "]")
  EndSelect    
...
Procedure ipdf_PutPages()
  Protected lwTotalPages,i,lsAnnots.s,lsRect.s,lsKids.s,lh.f,lsFilter.s,MEM.MEM_DataStructure
  
  lwTotalPages = pdfPage
  
  If (pdfAliasNbPages$ <> "")
    ; Replace number of pages
    For i = 1 To lwTotalPages 
      SelectElement(Pages(), i-1)
      MEM_DataReplace(Pages(), pdfAliasNbPages$, Str(lwTotalPages))
    Next
  EndIf
  ...
  For i = 1 To lwTotalPages 
    ...
    SelectElement(OrientationChanges(),i-1)
    ;If OrientationChanges() = #True
      SelectElement(PagesF(),i-1)
      ;ipdf_Out("/MediaBox [0 0 " + ipdf_StrF(pdfHPt,2) + " " + ipdf_StrF(pdfWPt,2) + "]")
      ipdf_Out("/MediaBox [0 0 " + ipdf_StrF(PagesF()\pdfWPt,2) + " " + ipdf_StrF(PagesF()\pdfHPt,2) + "]")
    ;EndIf
        ...
        Select PageLinks()\Type
          Case #PDF_URI_ACTIONS
            ...
            If (Trim(PageLinks()\Option1$) <> "")
              lsAnnots = lsAnnots + "/A <</S /URI /URI " + ipdf_TextString(PageLinks()\Option1$) + ">>>>"
            Else
              SelectElement(OrientationChanges(), PageLinks()\Page - 1)
              If OrientationChanges() = #True
                pdfH = pdfWPt
              Else
                pdfH = pdfHPt
              EndIf
              lsAnnots = lsAnnots + "/Dest [" + Str(1+2*PageLinks()\DestPage)
              lsAnnots = lsAnnots + " 0 R /XYZ 0 " + ipdf_StrF(pdfH - PageLinks()\y*pdfK,2)
              lsAnnots = lsAnnots + " null]>>"
            EndIf
            lsAnnots + #PDF_NEWLINE
    ...
    lsFilter=""
    If pCompress
      MEM\pData    = Pages()\pData
      MEM\lCurSize = Pages()\lCurSize
      MEM\lMaxSize = Pages()\lMaxSize
      If pdf_CallFunction(pCompress,MEM)
        ;FreeMemory(Pages()\pData)
        Pages()\pData    = MEM\pData
        Pages()\lCurSize = MEM\lCurSize
        Pages()\lMaxSize = MEM\lMaxSize
        lsFilter = "/Filter /FlateDecode "
      EndIf
    EndIf
    ipdf_NewObj()
    ipdf_Out("<<" + lsFilter + "/Length " + Str(Pages()\lCurSize) + ">>")
    ipdf_PutStream(Pages())
    ipdf_Out("endobj")
    If Pages()\pData
      FreeMemory(Pages()\pData)
    EndIf
  ...
  ;lsKids = ""
  ;For i=0 To lwTotalPages-1
  ;  lsKids = lsKids + Str(3 + 2*i) + " 0 R "
  ;Next
  
  ipdf_Out("/Kids [" + lsKids + "]")
  ipdf_Out("/Count " + Str(lwTotalPages))
  
  ;ipdf_Out("/MediaBox [0 0 " + ipdf_StrF(pdfWPt,2) + " " + ipdf_StrF(pdfHPt,2) + "]")
  ;SelectElement(PagesF(),0)
  ;ipdf_Out("/MediaBox [0 0 " + ipdf_StrF(PagesF()\pdfHPt,2) + " " + ipdf_StrF(PagesF()\pdfWPt,2) + "]")
...
Procedure ipdf_EndDoc()
  ...
  pdfJavaScript$=""
  ClearList(Pages())
  ClearList(PagesF()) ; found by fsw
  ClearList(Offsets())
  ClearList(OrientationChanges())
  ClearList(PageLinks())
  ClearList(Fonts())
  ClearList(FontList())
  ClearList(Images())
...
Procedure ipdf_Create(Orientation$="", Unit$="", Format$="")
  ...
  EndIf
  pdfBuffer\pData = 0
  If MEM_DataInit(pdfBuffer, 63999) = #False
    pdfError = #PDF_ERROR_OUT_OF_MEMORY
    ProcedureReturn #False
  EndIf
...
Procedure ipdf_SetProcHeader(ProcAddress)
  pHeader = ProcAddress
EndProcedure

Procedure ipdf_SetProcFooter(ProcAddress)
  pFooter = ProcAddress
EndProcedure
...
Procedure ipdf_Image(FileName$,*Mem,MemSize,x.f,y.f,w.f=0,h.f=0,link.l=0)
...
Procedure ipdf_BookMark(Text$, level.l=0, y.f=0)
  If y=-1 
    y = ipdf_GetY() 
  EndIf 
  AddElement(OutLines()) 
...
Procedure ipdf_PutBookMarks()
  Protected nb,ln,level,i,lParent,lPrev,h.f,k.f
  ...
  ForEach OutLines()
    ...
    h = ipdf_GetH()
    k = ipdf_GetK()
    ipdf_Out("/Dest [" + Str(1+2*OutLines()\wPage) + " 0 R /XYZ 0 " + ipdf_StrF((h-OutLines()\Y)*k,2) + " null]")
...
CompilerIf Defined(PurePDF_Include,#PB_Constant)
  ...
  Procedure pdf_BookMark(Text$, Level.l=0, Y.f=0) ;Add bookmark
  ...
  Procedure pdf_Image(FileName$, X.f, Y.f, W.f=0, H.f=0, Link.l=0) ;Puts an image in the page.
    ipdf_Image(FileName$,0,0,x,y,w,h,link)
  EndProcedure
  
  Procedure pdf_ImageMem(Name$,*Mem,MemSize, X.f, Y.f, W.f=0, H.f=0, Link.l=0) ;Puts an image in the page.
  ...
  Procedure pdf_SetY(Y.f) ;Moves the current abscissa back to the left margin and sets the ordinate.
    ipdf_SetY(Y)
  EndProcedure 
  ...
  Procedure pdf_SetProcHeader(ProcAddress) ;Set header procedure.
    ipdf_SetProcHeader(ProcAddress)
  EndProcedure
  
  Procedure pdf_SetProcFooter(ProcAddress) ;Set footer procedure.
  ...
  ProcedureDLL pdf_MirrorH(X.f=-1)
    ipdf_Scale(-100,100,X,-1)
  EndProcedure
  
  ProcedureDLL pdf_MirrorV(Y.f=-1)
...
CompilerElse
  ...
  ProcedureDLL pdf_SetY(Y.f) ;Moves the current abscissa back to the left margin and sets the ordinate.
    ipdf_SetY(Y)
  EndProcedure 
  ...
  ProcedureDLL pdf_EmbedFile(Filename$)
