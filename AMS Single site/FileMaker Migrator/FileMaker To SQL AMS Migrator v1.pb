...
;/ 
;/ Load AMS database into FileMaker Pro Advanced (v9+ ?)
;/ Customer AMS databases will be locked - require sending to Geoff to get guest account with full privileges.
;/    If Geoff has unlocked a customers AMS file, then there's no need for you to use the padlock unlock.
...
Procedure MakeConnection()
  connectionString$ = "dsn=LegacyAMS; User Id=troika;SLKStaticCursorLongColBuffLen=1024"
...
Procedure Init_Main()
  ...
  If #Debug = 1
    ...
    Frame3DGadget(#Gad_Frame_FMPro,400,4,1492,452,"FM Pro data")
    ...
    Frame3DGadget(#Gad_Frame_SQLite,4,456,1892,452,"SQLite data")
...
Procedure Get_RollID(Txt.s)
  ...
  ForEach RollMaster()
    If Trim(RollMaster()\Name) = Txt
      If Txt = "58546A01"
        Debug "58546A01 found at Record: "+Str(RollMaster()\ID)
      EndIf
...
Procedure Load_MasterData()
  ...
  If ADOmate_DatabaseQuery(myConnection, SQL$, #adOpenStatic)
    While ADOmate_IsEOF(myConnection) = 0
      ...
      RollMaster()\Name = StripSpecial(ADOmate_GetDatabaseString(myConnection,0))
      RollMaster()\Capacity = 100
      RollMaster()\Comments = Trim(ADOmate_GetDatabaseString(myConnection,7))
      ...
      RollMaster()\Wall = ADOmate_GetDatabaseLong(myConnection,1)
      RollMaster()\Opening = ADOmate_GetDatabaseLong(myConnection,2)
      RollMaster()\Operator = StripSpecial(ADOmate_GetDatabaseString(myConnection,16))
      
      Date.s = ADOmate_GetDatabaseString(myConnection,19)
      ...
      Date.s = ADOmate_GetDatabaseString(myConnection,6)
      Year = Val(StringField(Date,3,"/"))
      Month = Val(StringField(Date,2,"/"))
      Day = Val(StringField(Date,1,"/"))
      RollMaster()\ReadingDate = Date(Year,Month,Day,0,0,0)
      
      RollMaster()\ScreenCount = ADOmate_GetDatabaseLong(myConnection,4)
      RollMaster()\Vol1 = ADOmate_GetDatabaseFloat(myConnection,10)
      RollMaster()\Vol2 = ADOmate_GetDatabaseFloat(myConnection,11)
      RollMaster()\Vol3 = ADOmate_GetDatabaseFloat(myConnection,12)
      RollMaster()\Vol4 = ADOmate_GetDatabaseFloat(myConnection,13)
      RollMaster()\Vol5 = ADOmate_GetDatabaseFloat(myConnection,14)
      
      ;/ these require lookups:
      RollMaster()\Suitability = Get_SuitabilityID(ADOmate_GetDatabaseString(myConnection,8))
      RollMaster()\GroupID = Get_GroupID(ADOmate_GetDatabaseString(myConnection,9))
      RollMaster()\Manufacturer = Get_ManufacturerID(ADOmate_GetDatabaseString(myConnection,21))
    ...
    If ADOmate_DatabaseQuery(myConnection, SQL$, #adOpenStatic)
      While ADOmate_IsEOF(myConnection) = 0
        row + 1
        Record = Get_RollID(ADOMate_GetDatabaseString(myConnection,0))-1
        ;Debug "Record for Blob: "+ Str(Record)
        If Record > 0
        SelectElement(RollMaster(),Record)
        Rollmaster()\Blobmem = ADOmate_GetDatabaseBlob(myConnection,1)
        Rollmaster()\BlobSize = GlobalSize
        EndIf 
        ADOmate_NextDatabaseRow(myConnection)
      Wend
      ADOmate_FinishDatabaseQuery(myConnection)
...
Procedure Load_RollData()
  ...
  If ADOmate_DatabaseQuery(myConnection, SQL$, #adOpenStatic)
    ...
    While ADOmate_IsEOF(myConnection) = 0
      RollID.s = ADOmate_GetDatabaseString(myConnection,0)
      Record = Get_RollID(RollId)
      
      If RollID = "58546A01"
        Debug "Record: "+Str(Record)
      EndIf
      
      
      For i = 0 To numColumns-1
        a$ = ADOmate_DatabaseColumnName(myConnection, i)
        a$ + " ("+Str(ADOmate_DatabaseColumnType(myConnection, i))+") - "
        
        Select ADOmate_DatabaseColumnType(myConnection, i)
          Case 0
            a$+ADOmate_GetDatabaseString(myConnection,i)+Chr(10)
          Case #PB_Database_Long
            a$+Str(ADOmate_GetDatabaseLong(myConnection,i))+Chr(10)
          Case #PB_Database_String
            a$+ADOmate_GetDatabaseString(myConnection,i)+Chr(10)
          Case #PB_Database_Float
            a$+StrF(ADOmate_GetDatabaseFloat(myConnection,i),2)+Chr(10)
          Case #PB_Database_Double
            a$+StrD(ADOmate_GetDatabaseDouble(myConnection,i),2)+Chr(10)
          Case #PB_Database_Quad
            a$+Str(ADOmate_GetDatabaseQuad(myConnection,i))+Chr(10)
          Default
            a$+"Blob"+Chr(10)
        ...
        If RollID = "58546A01"
          ...
          Debug "Blah: "+a$ ;AddGadgetColumn(#Gad_RollContents,i,a$,Len(a$)*7)
        EndIf
      ...
      If Record > 0 ;/ has related master roll data
        ...
        RollData()\Operator = ADOmate_GetDatabaseString(myConnection,7)
        
        Date.s = ADOmate_GetDatabaseString(myConnection,1)
        Year = Val(StringField(Date,3,"/"))
        Month = Val(StringField(Date,2,"/"))
        Day = Val(StringField(Date,1,"/"))
        RollData()\ReadingDate = Date(Year,Month,Day,0,0,0)
        
        RollData()\RollID =Get_RollID( StripSpecial(ADOmate_GetDatabaseString(myConnection,0)))
        RollData()\Vol1 = ADOmate_GetDatabaseFloat(myConnection,2)
        RollData()\Vol2 = ADOmate_GetDatabaseFloat(myConnection,3) 
        RollData()\Vol3 = ADOmate_GetDatabaseFloat(myConnection,4) 
        RollData()\Vol4 = ADOmate_GetDatabaseFloat(myConnection,5) 
        RollData()\Vol5 = ADOmate_GetDatabaseFloat(myConnection,6) 
        
        RollData()\BlobMem = ADOmate_GetDatabaseBlob(myConnection,10)
        RollData()\BlobSize = GlobalSize
...
Procedure Load_AdminData()
  ...
  If ADOmate_DatabaseQuery(myConnection, SQL$, #adOpenStatic)
    ...
    Admin\Company = ADOmate_GetDatabaseString(myConnection,0)
    Admin\ScreenFormat = ADOmate_GetDatabaseString(myConnection,1)
    Admin\LengthUnit = ADOmate_GetDatabaseString(myConnection,2)
    Admin\VolumeUnit = ADOmate_GetDatabaseString(myConnection,3)
    Admin\Location = ADOmate_GetDatabaseString(myConnection,4)
    ...
    If #Debug = 1
      If ADOmate_DatabaseQuery(myConnection, "Select * from Admin", #adOpenStatic)
        numColumns = ADOmate_DatabaseColumns(myConnection)
        If numColumns
          a$ = ""
          For i = 0 To numColumns-1
...
Procedure Load_GroupData()
  ...
  If ADOmate_DatabaseQuery(myConnection, SQL$, #adOpenStatic)
    While ADOmate_IsEOF(myConnection) = 0
      ...
      Groups()\Name = ADOmate_GetDatabaseString(myConnection,0)
      Groups()\SiteID = 1
      Groups()\Type = 1
      ADOmate_NextDatabaseRow(myConnection)
    Wend
...
;  Manufacturers()\ID = 1 : Manufacturers()\Name = "Unknown"
  ...
  If ADOmate_DatabaseQuery(myConnection, SQL$, #adOpenStatic)
    While ADOmate_IsEOF(myConnection) = 0
      Txt.s = ADOmate_GetDatabaseString(myConnection,0)
...
Procedure Load_DamageData()
  ...
  If ADOmate_DatabaseQuery(myConnection, SQL$, #adOpenStatic)
    While ADOmate_IsEOF(myConnection) = 0
      ...
      Date.s = ADOmate_GetDatabaseString(myConnection,0)
      Year = Val(StringField(Date,3,"/"))
      Month = Val(StringField(Date,2,"/"))
      Day = Val(StringField(Date,1,"/"))
      Damage()\Date = Str(Date(Year,Month,Day,0,0,0))
      
      Damage()\DistanceFromLeft = Str(ADOmate_GetDatabaseFloat(myConnection,1))
      Damage()\DistanceFromRight = Str(ADOmate_GetDatabaseLong(myConnection,2))
      Damage()\RollID = Get_RollID(ADOmate_GetDatabaseString(myConnection,3))
      Damage()\Size = StrF(ADOmate_GetDatabaseFloat(myConnection,4),2)
      Damage()\Type = ADOmate_GetDatabaseString(myConnection,5)
...
Procedure Migrate()
  ...
  If OpenDatabase(1,"Database\AMS_SS.db","","",#PB_Database_SQLite)
    ...
    ForEach RollMaster()
      ...
      SetDatabaseBlob(1,0,RollMaster()\BlobMem,Rollmaster()\BlobSize)
      With RollMaster()
    ...
    ForEach RollData()
      ...
      ;/        1          2            3      4    5      6    7      8           9         
      SetDatabaseBlob(1,0,RollData()\BlobMem,RollData()\BlobSize)
      With RollData()
...
; FirstLine = 33
; Folding = CAAAAA9
; Executable = Filemaker to SQL AMS migrator.exe
; EnablePurifier