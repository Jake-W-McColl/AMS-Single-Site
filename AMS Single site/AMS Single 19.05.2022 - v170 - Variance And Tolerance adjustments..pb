...
If Multi_Site_Mode = 0
  Program_Version.s = " V1.70 - Single-Site"  ;/DNT
Else
  Program_Version.s = " V1.70 - Multi-Site" ;/DNT
EndIf
...
Procedure SendToAQCIni(RollName.s, RollID.i)
  Protected lineTxt.s, Arg.s, Val.i, Path.s, Size.i, Mem.i, AllText.s, OldText.s, LineNumber.i
  ...
  If ReadFile(0, Path+"Anilox QC.ini", #PB_Unicode)
    Repeat
      ...
      Val = Val(Trim(StringField(LineTxt,2,"=")))
      Select Arg.s
        Case "ROLLID"
  ...
  If ReadFile(0, Path+"Anilox QC_HD.ini", #PB_Unicode)
    Size = Lof(0)
    Mem = AllocateMemory(Size)
    If Mem
      ReadData(0, Mem, Size)
  ...
  If RollLineNum>0
    ...
    AllText = ReplaceString(AllText, OldText, "Sections="+SectionCount)
    Debug(AllText)
    If CreateFile(0, Path+"Anilox QC_HD.ini", #PB_Unicode)
      Val = WriteData(0, @AllText, Len(AllText)*2)
      Debug(Val)
    CloseFile(0)
    EndIf
...
Procedure Init_Settings() ;/ Last Updated PJ 06.01.2022
  ...
  X = 210 : Y = 24
  ;/ PJ - removed as part of v1.68 update
          ...
          Case #Settings_Suitability_New
            ...
            If ResultS <> ""
              ...
              If Resulti = 0
                ...
                ;/SelectElement(SuitabilityList(),DatabaseID)
                ;/MessageRequester(Str(SuitabilityList()\id),Str(DatabaseID))
